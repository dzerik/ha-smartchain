...
Что это даёт — **фильтрацию**: `search_entities(query="…", state="open")` превращается в фильтр по метаданным, вычисляемый внутри векторного хранилища. Чего это не даёт — лучшего поиска. Косинусная близость по `"on"`, `"off"` и `"23.5"` слаба, и режим существует не ради неё.

**Оставить выключенным ничего не стоит в смысле свежести.** Состояние, которое показывает `search_entities`, в обоих режимах читается вживую из `hass.states` в момент ответа, поэтому оно никогда не устаревает. Передать `state=` хранилищу с `index_states: false` тоже не ошибка — фильтр просто применится после живого чтения, а не внутри хранилища, и вызывающий получит тот же ответ.
