...
Учтите: запрос здесь — целая фраза пользователя, а не поисковое выражение, составленное моделью, поэтому сопоставление идёт **и целиком, и по словам**: «включи свет на кухне» не называется у вас в доме ничем, а «свет» — называется. Слова короче трёх символов отбрасываются («на», «то», «in», «on» — подстрока половины дома), а попадание по всей фразе всё равно опережает попадание по слову, так что устройство, чьё имя совпало с запросом целиком, идёт первым. `search_entities` (§9.10) по-прежнему сопоставляет свой `query` целиком: его модель выбирает намеренно.

Настроенный индекс сущностей (§9.10) добавляет сверху второй проход: векторный поиск по хранилищу, слитый с лексическими попаданиями тем же ранжированием, каким пользуется `search_entities`, — сперва точные лексические, затем префиксные, затем векторные по score, с дедупликацией по `entity_id`. Именно он находит сущность, у имени которой нет с запросом ни одного общего слова. Векторный проход выполняется, только если индекс сущностей **ровно один**; при двух и более выбрать без произвола нельзя, а спросить некого, поэтому поиск остаётся лексическим, а не отдаёт молча предпочтение одному индексу перед другим.
