...
Сервис `smartchain.ask` не нашёл агента. Проверьте **Настройки > Устройства и службы > SmartChain** — есть ли хотя бы один агент диалога с заполненным `runtime_data`?
